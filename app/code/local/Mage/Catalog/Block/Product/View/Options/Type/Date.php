...
class Mage_Catalog_Block_Product_View_Options_Type_Date extends Mage_Catalog_Block_Product_View_Options_Abstract
{
	public function getDateFormat() {
		$format = Mage::app()->getLocale()->getTranslation(Mage_Core_Model_Locale::FORMAT_TYPE_SHORT, 'date');
		
		$data = array(
			"/".Zend_Date::MONTH_NAME."/" => "MM",
			"/".Zend_Date::MONTH_NAME_SHORT."/" => "M",
			"/".Zend_Date::MONTH."/" => "mm",
			"/".Zend_Date::MONTH_SHORT."/" => "m",

			"/".Zend_Date::YEAR."/" => "yy",
			"/".Zend_Date::YEAR_SHORT."/" => "y",
			"/".Zend_Date::YEAR_8601."/" => "yy",
			"/".Zend_Date::YEAR_SHORT_8601."/" => "y"
		);

		return preg_replace(array_keys($data), array_values($data), $format);
	}

	public function getDayNames() {
		return Mage::app()->getLocale()->getTranslationList("days")["format"]["wide"];
	}

	public function getDayNamesAbbr() {
		return Mage::app()->getLocale()->getTranslationList("days")["format"]["abbreviated"];
	}

	public function getMonthNames() {
		return Mage::app()->getLocale()->getTranslationList("months")["format"]["wide"];
	}

	public function getMonthNamesAbbr() {
		return Mage::app()->getLocale()->getTranslationList("months")["format"]["abbreviated"];
	}

	public function getFirstWDay() {
		return (int)Mage::getStoreConfig('general/locale/firstday');
	}
