...
class Mage_Customer_Model_Address_Abstract extends Mage_Core_Model_Abstract
{
    ...
    protected function _basicCheck()
    {
        ...
        if ($egovsHelper->isFieldRequired('firstname', 'register') && !Zend_Validate::is($this->getFirstname(), 'NotEmpty')) {
        	$this->addError($helper->__('Please enter the first name.'));
        }
        
        if ($egovsHelper->isFieldRequired('lastname', 'register') && !Zend_Validate::is($this->getLastname(), 'NotEmpty')) {
        	$this->addError($helper->__('Please enter the last name.'));
        }
        
        if ($egovsHelper->isFieldRequired('street', 'register') && !Zend_Validate::is($this->getStreet(1), 'NotEmpty')) {
        	$this->addError($helper->__('Please enter the street.'));
        }
        
        if ($egovsHelper->isFieldRequired('city', 'register') && !Zend_Validate::is($this->getCity(), 'NotEmpty')) {
        	$this->addError($helper->__('Please enter the city.'));
        }
        
        if ($egovsHelper->isFieldRequired('telephone', 'register') && !Zend_Validate::is($this->getTelephone(), 'NotEmpty')) {
        	$this->addError($helper->__('Please enter the telephone number.'));
        }
        
        $_havingOptionalZip = Mage::helper('directory')->getCountriesWithOptionalZip();
        if ($egovsHelper->isFieldRequired('postcode', 'register') && !in_array($this->getCountryId(), $_havingOptionalZip)) {
        ...
        if ($egovsHelper->isFieldRequired('country_id', 'register') && !Zend_Validate::is($this->getCountryId(), 'NotEmpty')) {
        	$this->addError($helper->__('Please enter the country.'));
        }
        
        if ($egovsHelper->isFieldRequired('region', 'register')
        		&& $this->getCountryModel()->getRegionCollection()->getSize()
        		&& !Zend_Validate::is($this->getRegionId(), 'NotEmpty')
        ) {
        	$this->addError($helper->__('Please enter the state/province.'));
        }
