 ...
 */
class Mage_Bundle_Block_Catalog_Product_View_Type_Bundle_Option extends Egovs_Ready_Block_Bundle_Catalog_Product_Price
{
    /**
    ...
    public function formatPriceString($price, $includeContainer = true)
    {
        ...
        $formattedTaxRate = $this->getFormattedTaxRate();
        $taxPostfix = '';
        if ($formattedTaxRate === true) {
            switch (Mage::helper('tax')->getPriceDisplayType()) {
                case Mage_Tax_Model_Config::DISPLAY_TYPE_EXCLUDING_TAX:
                    $taxPostfix .= '<span class="tax-details">' . $this->__('excl. Tax') . '</span>';
                    break;
                case Mage_Tax_Model_Config::DISPLAY_TYPE_INCLUDING_TAX:
                    $taxPostfix .= '<span class="tax-details">' . $this->__('incl. Tax') . '</span>';
                    break;
                default:
            }
        } elseif ($formattedTaxRate === 0) {
            $taxPostfix .= '<span class="tax-details">' . $this->__('Tax Free') . '</span>';
        }
        if (!empty($taxPostfix)) {
            if (!$includeContainer) {
                $taxPostfix = sprintf('(%s)', $taxPostfix);
            }
            $formated .= '&nbsp;'.$taxPostfix;
        }
    ...
    protected function _toHtml() {
        return Mage_Bundle_Block_Catalog_Product_Price::_toHtml(); // TODO: Change the autogenerated stub
    }
}
