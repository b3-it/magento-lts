...
class Symmetrics_SecurePassword_Model_Observer
{
    ...
    public function setCustomerLockStatus($customer)
    {
        ...
        if ($attemptLock && $timeLock && !$unlocked) {
        	$storeName  = Mage::getStoreConfig('general/imprint/shop_name');
        	$storePhone = Mage::getStoreConfig('general/imprint/telephone');
        	$storeEmail = Mage::getStoreConfig('general/imprint/email');

            $_error_msg = Mage::helper('securepassword')->__('Your account is locked due to too many failed login attempts.<br />Please contact the %s (fon. %s or email %s).',
            		                                         $storeName, $storePhone, $storeEmail);
        	throw new Exception($_error_msg);
        }
    }
