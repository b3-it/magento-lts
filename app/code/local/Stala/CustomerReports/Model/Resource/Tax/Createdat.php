 ...
 */
class Stala_CustomerReports_Model_Resource_Tax_Createdat  extends Mage_Tax_Model_Resource_Report_Tax_Createdat
{
    ...
    protected function _aggregateByOrder($aggregationField, $from, $to)
    {
        ...
        try {
            ...
            $select->group(array('period', 'code', 'percent', 'order_status','customer_group_id'));
            $insertQuery = $writeAdapter->insertFromSelect($select, $this->getMainTable(), array_keys($columns));
            $writeAdapter->query($insertQuery);
            $writeAdapter->commit();
        } catch (Exception $e) {
