 ...
 */
class Bkg_License_Model_Textprocess extends Bkg_License_Model_Abstract
{
	...
	protected function _replaceVariables($html) {
		$data = $this;
		...
		foreach ($ausgabe as $treffer) {
			...
			if ($value !== null) {
				if(is_array($value))
				{
					...
					{
						$this->setItem($item);
						$linehtml .= $this->_replaceVariables($parentline);
					}
