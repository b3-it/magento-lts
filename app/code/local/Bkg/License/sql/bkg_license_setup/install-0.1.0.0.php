...
if (!$installer->tableExists($installer->getTable('bkg_license/master')))
{
	...
	CREATE TABLE {$installer->getTable('bkg_license/master')} (
	  `id` int(11) unsigned NOT NULL auto_increment,
        `usetypeoption_id` int(11) unsigned NOT NULL,
	  ...
	  PRIMARY KEY (`id`),
      FOREIGN KEY (`usetypeoption_id`) REFERENCES `{$this->getTable('bkg_tollpolicy/use_options_entity')}`(`id`) ON DELETE CASCADE
...
if (!$installer->tableExists($installer->getTable('bkg_license/master_text')))
{
	$installer->run("
	-- DROP TABLE IF EXISTS {$installer->getTable('bkg_license/master_text')};
	CREATE TABLE {$installer->getTable('bkg_license/master_text')} (
	  `id` int(11) unsigned NOT NULL auto_increment,
    `master_id` int(11) unsigned NOT NULL,
        `identifier` varchar(128) default '',
        `pos` int(11) default '0',
