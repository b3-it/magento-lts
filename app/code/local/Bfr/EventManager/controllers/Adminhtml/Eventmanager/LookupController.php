 ...
 * @name       	Bfr_EventManager_Adminhtml_LookupController
 * @author 		Holger K�gel <h.koegel@b3-it.de>
 * @copyright  	Copyright (c) 2015 B3 It Systeme GmbH - http://www.b3-it.de
 * @license		http://sid.sachsen.de OpenSource@SID.SACHSEN.DE
 */
class Bfr_EventManager_Adminhtml_EventManager_LookupController extends Mage_Adminhtml_Controller_action
{

	protected function _initAction() {
		$this->loadLayout()
			->_setActiveMenu('lookup/items')
	...
	public function editAction() {
		...
		$model  = Mage::getModel('eventmanager/lookup')->load($id);
		...
		if ($model->getId() || $id == 0) {
			...
			Mage::register('lookup_data', $model);
			...
			$this->_addContent($this->getLayout()->createBlock('eventmanager/adminhtml_lookup_edit'))
				->_addLeft($this->getLayout()->createBlock('eventmanager/adminhtml_lookup_edit_tabs'));
	...
	public function saveAction() {
		if ($data = $this->getRequest()->getPost()) {
			...
			$model = Mage::getModel('eventmanager/lookup');
			$model->setData($data)
    ...
    public function massDeleteAction() {
        $lookupIds = $this->getRequest()->getParam('lookup');
        if(!is_array($eventmanagerIds)) {
			Mage::getSingleton('adminhtml/session')->addError(Mage::helper('adminhtml')->__('Please select item(s)'));
        } else {
            try {
                foreach ($eventmanagerIds as $eventmanagerId) {
                    $eventmanager = Mage::getModel('eventmanager/lookup')->load($eventmanagerId);
                    $eventmanager->delete();
                }
        ...
        if(!is_array($eventmanagerIds)) {
            ...
            try {
                foreach ($lookupIds as $lookupId) {
                    $lookup = Mage::getSingleton('eventmanager/lookup')
                        ->load($eventmanagerId)
    ...
    {
        $fileName   = 'lookup.csv';
        $content    = $this->getLayout()->createBlock('eventmanager/adminhtml_lookup_grid')
            ->getCsv();
