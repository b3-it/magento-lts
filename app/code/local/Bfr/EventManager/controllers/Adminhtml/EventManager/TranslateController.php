 ...
 * @name       	Bfr_EventManager_Adminhtml_TranslateController
 * @author 		Holger K�gel <h.koegel@b3-it.de>
 * @copyright  	Copyright (c) 2015 B3 It Systeme GmbH - http://www.b3-it.de
 * @license		http://sid.sachsen.de OpenSource@SID.SACHSEN.DE
 */
class Bfr_EventManager_Adminhtml_EventManager_TranslateController extends Mage_Adminhtml_Controller_action
{
	...
	public function editAction() {
		...
		$model  = Mage::getModel('eventmanager/translate')->load($id);
		...
		if ($model->getId() || $id == 0) {
			...
			$this->_addContent($this->getLayout()->createBlock('eventmanager/adminhtml_translate_edit'))
				->_addLeft($this->getLayout()->createBlock('eventmanager/adminhtml_translate_edit_tabs'));
	...
	public function saveAction() {
		if ($data = $this->getRequest()->getPost()) {
			...
			$model = Mage::getModel('eventmanager/translate');
			$model->setData($data)
        ...
        if(!is_array($eventmanagerIds)) {
            ...
            try {
                foreach ($eventmanagerIds as $eventmanagerId) {
                    $eventmanager = Mage::getModel('eventmanager/translate')->load($eventmanagerId);
                    $eventmanager->delete();
                }
        ...
        if(!is_array($eventmanagerIds)) {
            ...
            try {
                foreach ($translateIds as $translateId) {
                    $translate = Mage::getSingleton('eventmanager/translate')
                        ->load($eventmanagerId)
    ...
    public function exportCsvAction()
    {
        ...
        $content    = $this->getLayout()->createBlock('eventmanager/adminhtml_translate_grid')
            ->getCsv();
