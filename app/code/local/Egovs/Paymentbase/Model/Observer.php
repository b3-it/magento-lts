...
// 			Mage::log('sql::'.$collection->getSelect()->assemble(), Zend_Log::DEBUG, Egovs_Helper::LOG_FILE);
    ...
    public function runAutomaticPaymentRetrieval($schedule) {
        ...
        foreach ($aprEnabledByStore as $storeId=>$aprEnabled) {
            if (!$aprEnabled) {
                Mage::log('apr::APR for store %s not enabled, omitting!', Zend_Log::DEBUG, Egovs_Helper::LOG_FILE);
                continue;
            }
            ...
            try {
                ...
                if (!$schedule->trySchedule(time())) {
                    Mage::log("apr::APR for store ID $storeId not scheduled for now cron expr: {$schedule->getCronExpr()}, omitting!", Zend_Log::DEBUG, Egovs_Helper::LOG_FILE);
                    continue;
                }
