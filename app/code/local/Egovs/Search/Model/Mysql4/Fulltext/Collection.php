...
class Egovs_Search_Model_Mysql4_Fulltext_Collection extends Mage_CatalogSearch_Model_Mysql4_Fulltext_Collection
{
    /**
     * @var bool
     */
     ...
     * @return  Egovs_Search_Model_Mysql4_Fulltext_Collection
     */
    public function addSearchFilter($query)
    {
        ...
        if (strlen($phonquery) > 1) {
            ...
            $this->getSelect()
                /*
                 * JoinLeft Fulltext, where fulltext like, to get all hits outside of soundex
                 */
                ->joinLeft(array('soundex_result' => $this->getTable('egovssearch/soundex')), 'soundex_result.product_id=e.entity_id', array())
                ->joinLeft(array('fulltext' => 'catalogsearch_fulltext'), 'fulltext.product_id=e.entity_id')
                ->where($exp)
                ->orWhere($SoundExpr)
                ->orWhere('lower(fulltext.data_index) like ?', '%' . $query . '%');
