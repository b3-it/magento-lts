<?php
/**
 * Klasse für ePayment-Kommunikation im Saferpay-Verfahren (Giropay).
 *
 * @category	Egovs
 ...
 */
class Egovs_Zahlpartnerkonten_Model_Payment_Giropay extends Egovs_Giropay_Model_Giropay
{
	const MAX_RETRIES = 1;
    ...
    protected function _authorize(Varien_Object $payment, $amount) {
    	...
    	if (Mage::helper('zpkonten')->isUseZpkonto($this->_getOrder()->getCustomer())) {
    		Mage::log("giropay::Zahlpartnerkonto : Customers uses Zahlpartnerkonten, OrderID: {$this->_getOrder()->getIncrementId()}", Zend_Log::INFO, Egovs_Helper::LOG_FILE);
    		$payment->setKassenzeichen($this->_getKassenzeichen());
    		Mage::log("giropay::KASSENZEICHEN :{$payment->getKassenzeichen()}, OrderID: {$this->_getOrder()->getIncrementId()}", Zend_Log::INFO, Egovs_Helper::LOG_FILE);
    	} else {
    		Mage::log("giropay::Zahlpartnerkonto : Customers doesn't use Zahlpartnerkonten we don't set a Kassenzeichen now, OrderID: {$this->_getOrder()->getIncrementId()}", Zend_Log::INFO, Egovs_Helper::LOG_FILE);
    	}
    	/*
