 ...
 */
class Egovs_Shipment_Model_Resource_Carrier_Bulkgoods extends Mage_Core_Model_Resource_Db_Abstract
{
    /**
    ...
    {
        $this->_init('egovsshipment/bulkgoods', 'pk');
    }
...
//die($select->assemble());
    ...
    {
        if (empty($_FILES['groups']['tmp_name']['bulkgoods']['fields']['import']['value'])) {
            return $this;
        }

        $csvFile = $_FILES['groups']['tmp_name']['bulkgoods']['fields']['import']['value'];
        ...
        $CsvDelimiter = ',';
        if(Mage::getStoreConfig('admin/export/csv_separator',0))
        {
        	$CsvDelimiter =  Mage::getStoreConfig('admin/export/csv_separator',0);
        }
        // check and skip headers
        $headers = $io->streamReadCsv($CsvDelimiter);
        if ($headers === false || count($headers) < 5) {
            $io->streamClose();
            Mage::throwException(Mage::helper('shipping')->__('Invalid Bulkgoods File Format'));
        }
        ...
        try {
            ...
            while (false !== ($csvLine = $io->streamReadCsv($CsvDelimiter))) {
                $rowNumber ++;
            ...
            $io->streamClose();
            Mage::logException($e);
            Mage::throwException(Mage::helper('shipping')->__('An error occurred while import bulkgoods.'));
        }
    ...
    protected function _getImportRow($row, $rowNumber = 0)
    {
        ...
        // validate region
        if ($countryId != '0' && isset($this->_importRegions[$countryId][$row[2]])) {
            $regionId = $this->_importRegions[$countryId][$row[2]];
        } elseif ($row[2] == '*' || $row[2] == '') {
            $regionId = 0;
        } else {
            $this->_importErrors[] = Mage::helper('shipping')->__('Invalid Region/State "%s" in the Row #%s.', $row[2], $rowNumber);
            return false;
        }
...
//         }
        ...
        if (!is_numeric($row[3])) {
        	$this->_importErrors[] = Mage::helper('shipping')->__('Invalid Number "%s" in the Row #%s.', $row[3], $rowNumber);
        	return false;
        }
        // anzahl
        $number = intval($row[3]);
        ...
        $hash = sprintf("%s-%s-%d-%F-%d",$group,$countryId, $regionId, $price, $number);
        if (isset($this->_importUniqueHash[$hash])) {
            $this->_importErrors[] = Mage::helper('shipping')->__('Duplicate Row #%s (Country "%s", Region/State "%s" and Value "%s").', $rowNumber, $row[1], $row[2], $value);
            return false;
        }
