...
class Egovs_Extstock_Block_Adminhtml_Extstock_Edit_Tab_Status extends Mage_Adminhtml_Block_Widget_Form
{
	protected function _prepareForm()
	{
		...
		try
		{
			...
			*/
        	...
        	$resourceId = $session->getData('acl')->get("admin/extstock/extstockorderlist/instock")->getResourceId();
        	$canInStock = $session->isAllowed($resourceId);
        	
        	$resourceId = $session->getData('acl')->get("admin/extstock/extstockorderlist/extstockstatus")->getResourceId();
        	$canStatus = $session->isAllowed($resourceId);
		...
		if($canStatus && Mage::registry('extstock_data')->getOrigData('status') != Egovs_Extstock_Helper_Data::DELIVERED)
		{
			...
			$fieldset->addField('status', 'select', array(
		        ...
		        'values'    => array(
					array(
						'value'     => Egovs_Extstock_Helper_Data::ORDERED,
			            'label'     => Mage::helper('extstock')->__('Ordered'),
					),
					array(
		                'value'     => Egovs_Extstock_Helper_Data::DELIVERED,
