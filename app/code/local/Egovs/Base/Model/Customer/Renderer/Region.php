...
class Egovs_Base_Model_Customer_Renderer_Region implements Varien_Data_Form_Element_Renderer_Interface
{
    ...
    public function render(Varien_Data_Form_Element_Abstract $element)
    {
        ...
        // hidden input need to be disabled too  if element is disabled
        $disableAttributes = array('disabled', 'readonly');
        ...
        if ($regionCollection && count($regionCollection) > 0) {
            ...
            $html .= '<select id="' . $regionIdHtmlId . '" name="' . $regionIdHtmlName . '" '
                . $element->serialize($htmlAttributes + $disableAttributes) .'>' . "\n";
            ...
            $html .= '<input type="hidden" name="' . $regionHtmlName . '" id="' . $regionHtmlId . '" value="" '. $element->serialize($disableAttributes) . '/>';
            ...
            $html .= '<input type="hidden" name="' . $regionIdHtmlName . '" id="' . $regionIdHtmlId . '" value="" '. $element->serialize($disableAttributes) . '/>';
            $html .= '</td>'."\n";
        }
        $html.= '</tr>'."\n";
        return $html;
