 ...
 */
class Egovs_Isolation_Helper_Store extends Varien_Object
{
    ...
    {
        $this->_websiteCollection = Mage::helper('isolation')->getUserWebsites();
        return $this;
    }
        ...
        if($storegroups && (count($storegroups) > 0))
        {
	        foreach (Mage::app()->getWebsites() as $website) {
	        	foreach ($website->getGroups() as $group) {
	            	if(in_array($group->getId(), $storegroups)){
