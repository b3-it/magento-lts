...
class Slpb_Extstock_Block_Adminhtml_Extstock_Edit_Tab_Order extends Mage_Adminhtml_Block_Widget_Form
{
...
//	        'class'     => 'required-entry',
		...
		$fieldset->addField('stock_id', 'select', array(
		 ...
		 if (array_key_exists(Slpb_Extstock_Helper_Data::REORDER, $this->_attributes)) {
		 	foreach ($fieldset->getElements() as $element) {
		 		if (!$element)
		 			continue;
		 		$id = $element->getId();
		 		$element->setId(Slpb_Extstock_Helper_Data::REORDER."_".$id);
		 ...
		 if($data->getOrigData('status') == Slpb_Extstock_Helper_Data::DELIVERED
		    && !array_key_exists(Slpb_Extstock_Helper_Data::REORDER, $this->_attributes)) {
		 	foreach ($fieldset->getElements() as $element) {
		 		if (!$element)
		 			continue;
		 		$id = $element->getId();
