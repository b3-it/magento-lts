...
class Slpb_Extstock_Block_Adminhtml_Extstock_Edit_Tabs extends Mage_Adminhtml_Block_Widget_Tabs
{
	...
	protected function _beforeToHtml()
	{
		...
		if(Mage::registry('extstock_data')->getOrigData('status') != Slpb_Extstock_Helper_Data::ORDERED) {
		...
		if ($this->_isStockMode()) {
			$this->addTab('reorder_section', array(
	          ...
	          'content'   => $this->getLayout()->createBlock('extstock/adminhtml_extstock_edit_tab_order',
	      													 '',
															 array(Slpb_Extstock_Helper_Data::REORDER => 1)
															)->toHtml(),
			));
		}
